...
onMounted(async () => {
  ...
  try {
    ...
    if (clientData) {
      form.clientName = clientData.clientName;
      form.description = clientData.description;
      form.redirectUris = clientData.redirectUris?.length ? clientData.redirectUris : [''];
      form.authorizationGrantTypes = clientData.authorizationGrantTypes || [];
...
function addUri() { 
  if (form.redirectUris.length < 10) { // Limit URIs
...
async function handleSubmit() {
    ...
    try {
        await clientStore.updateClient(clientId, payload);
